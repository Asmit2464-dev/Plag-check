...
function HighlightedText({ originalText = '', highlights = [] }) {
  const flaggedSentences = new Set(
    highlights.map(({ input_sentence }) => input_sentence?.trim()).filter(Boolean)
  );
  ...
  return (
    <p style={{ whiteSpace: 'pre-wrap', lineHeight: 1.8, margin: 0 }}>
      {sentences.map((sentence, index) => {
        const isFlagged = flaggedSentences.has(sentence.trim());
        return (
          <span
            key={`${index}-${sentence.slice(0, 20)}`}
            style={isFlagged ? { background: 'rgba(244, 63, 94, 0.25)', borderRadius: 4, padding: '0.1rem 0.2rem' } : undefined}
          >
            {sentence}
          </span>
        );
